...
from ltp_dict_model import MultitaskGAT
from DataSets_ldm import MyDataSet
...
def train_model(train_dataloader, tags_size, num_sentiment_classes, all_sentiment_classes,
    ...
    scheduler = OneCycleLR(optimizer, max_lr=learning_rate, total_steps=num_epochs * len(train_dataloader[0]))
    # 定义损失函数的loss返回权重
    ...
    for epoch in range(num_epochs):
        ...
        # 定义进度条
        per = tqdm(enumerate(range(len(train_dataloader[0]))), total=len(train_dataloader[0]),
                   desc="第{}轮训练".format(epoch + 1),
                   ncols=150, position=0, dynamic_ncols=True)

        for i, j in per:
            data_ltp = train_dataloader[0][j].to(device)
            data_dict = train_dataloader[1][j].to(device)
            data = train_dataloader[0][j].to(device)
            # 清空梯度
            if (i + 1) % accumulation_steps == 0:
                optimizer.zero_grad()
            # 前向传播
            entity_loss, entity_output, sentiment_output, all_output = model(data_ltp, data_dict)
...
def test_model(test_dataloader, model, device):
    ...
    logging.info("模型测试:")
    with torch.no_grad():
        per = tqdm(enumerate(range(len(test_dataloader[0]))), total=len(test_dataloader[0]), desc="测试:", ncols=150,
                   position=0, dynamic_ncols=True)
        for i, j in per:
            data_ltp = test_dataloader[0][j].to(device)
            data_dict = test_dataloader[1][j].to(device)
            data = test_dataloader[0][j].to(device)
            entity_output, sentiment_output, all_output = model(data_ltp, data_dict)
            # 预测实体
...
def read_data(is_bio, bert_path, ltp_path, train_data_path, test_data_path):
    ...
    # 读取数据
    bert_input, edge_index, batch, y_entity, y_sentiment, all_sentiment, total_length = get_data_ldm(train_org_data,
                                                                                                     bert_path,
                                                                                                     ltp_path, is_bio)
    train_data = MyDataSet(bert_input, edge_index, batch, y_entity, y_sentiment, all_sentiment, total_length,
                           True)
    bert_input, edge_index, batch, y_entity, y_sentiment, all_sentiment, total_length = get_data_ldm(test_org_data,
                                                                                                     bert_path,
                                                                                                     ltp_path, is_bio)
...
if __name__ == '__main__':
    ...
    if is_bio:
        ...
        tags_size = 3
    # json数据读取
    train_dataloader, test_dataloader = read_data(is_bio, bert_path, ltp_path, train_data_path, test_data_path)
